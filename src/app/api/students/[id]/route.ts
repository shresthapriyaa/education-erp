...
const STUDENT_SELECT = {
  ...
  phone: true, address: true, img: true,
  bloodGroup: true, sex: true, dateOfBirth: true,
...
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const cleanParentId = body.parentId?.trim() || null;
    const cleanClassId  = body.classId?.trim()  || null;

    if (cleanParentId) {
      const exists = await prisma.parent.findUnique({ where: { id: cleanParentId }, select: { id: true } });
      if (!exists) return NextResponse.json({ error: `Parent not found` }, { status: 404 });
    }
    ...
    const student = await prisma.student.update({
      ...
      data: {
        ...
        sex:         body.sex,
        dateOfBirth: body.dateOfBirth ? new Date(body.dateOfBirth) : null,
        class:  cleanClassId  ? { connect: { id: cleanClassId  } } : { disconnect: true },
        parent: cleanParentId ? { connect: { id: cleanParentId } } : { disconnect: true },
      },
      select: STUDENT_SELECT,
    });
    ...
    if (error.code === "P2002") return NextResponse.json({ error: `Already exists: ${error.meta?.target}` }, { status: 409 });
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    const body = await req.json();
    if (Object.keys(body).length === 0) return NextResponse.json({ error: "No fields provided" }, { status: 400 });

    const cleanParentId = "parentId" in body ? body.parentId?.trim() || null : undefined;
    const cleanClassId  = "classId"  in body ? body.classId?.trim()  || null : undefined;
