...
//     // FIX: treat "none", undefined, and null all as "clear the teacher"
//     const classTeacherId =
//       body.classTeacherId && body.classTeacherId !== "none"
//         ? body.classTeacherId
//         : null;

//     // FIX: only validate teacher existence if we actually have an ID
//     if (classTeacherId) {
//       const teacherExists = await prisma.teacher.findUnique({ where: { id: classTeacherId } });
//       if (!teacherExists) return NextResponse.json({ error: "Teacher not found" }, { status: 404 });
//     }
...
//         section:        body.section?.trim() || existing.section,
//         academicYear:   body.academicYear?.trim() || existing.academicYear,
//         classTeacherId, // FIX: use the cleaned value (null clears the relation)
//       },
//       include: {
//         classTeacher: { select: { id: true, username: true, email: true } },
//         school: { select: { id: true, name: true } },
...
//     // FIX: same sentinel handling for PATCH
...
//     });
//     if (!c) return NextResponse.json({ error: "Class not found" }, { status: 404 });
//     return NextResponse.json(c, { headers: { "Cache-Control": "no-store, no-cache, must-revalidate" } });
//   } catch (error: any) {
//     return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
//   }
// }
...
//     const existing = await prisma.class.findUnique({ where: { id } });
//     if (!existing) return NextResponse.json({ error: "Class not found" }, { status: 404 });
//     const classTeacherId = body.classTeacherId && body.classTeacherId !== "none" ? body.classTeacherId : null;
//     if (classTeacherId) {
//       const teacherExists = await prisma.teacher.findUnique({ where: { id: classTeacherId } });
//       if (!teacherExists) return NextResponse.json({ error: "Teacher not found" }, { status: 404 });
//     }
...
//     if (body.academicYear !== undefined) data.academicYear = body.academicYear.trim();
//     if (body.classTeacherId !== undefined) {
//       data.classTeacherId = body.classTeacherId && body.classTeacherId !== "none" ? body.classTeacherId : null;
//     }
//     if (body.grade || body.section) {
//       data.name = `${body.grade || existing.grade} - ${body.section || existing.section}`;
//     }
...
//     await prisma.exam.deleteMany({ where: { classId: id } });
//     await prisma.result.deleteMany({ where: { classId: id } });
//     await prisma.routine.deleteMany({ where: { classId: id } });
//     await prisma.attendance.deleteMany({ where: { classId: id } });
//     await prisma.classSubject.deleteMany({ where: { classId: id } });
