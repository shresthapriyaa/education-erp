...
export async function PUT(
  ...
  try {
    ...
    const existing = await prisma.accountant.findUnique({ where: { id } });
    if (!existing) return NextResponse.json({ error: "Accountant not found" }, { status: 404 });

    if (email && email !== existing.email) {
      const emailExists = await prisma.accountant.findUnique({ where: { email } });
    ...
    console.error("[ACCOUNTANT_PUT]", error.message);
    if (error.code === "P2002") {
...
export async function PATCH(
  ...
  try {
    ...
    console.error("[ACCOUNTANT_PATCH]", error.message);
    if (error.code === "P2002") {
      return NextResponse.json({ error: `Already exists: ${error.meta?.target}` }, { status: 409 });
    }
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
...
export async function DELETE(
  ...
  try {
    ...
    if (accountant.userId) await prisma.user.delete({ where: { id: accountant.userId } });

    return NextResponse.json({ message: "Accountant deleted successfully" });
  } catch (error: any) {
    console.error("[ACCOUNTANT_DELETE]", error.message);
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
  }
}